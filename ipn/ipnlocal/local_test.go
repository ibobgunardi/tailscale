...
func TestNotifyForSessionPeerWireGuardStateVisibility(t *testing.T) {
	b := newTestLocalBackend(t)
	notify := ipn.Notify{
		PeerState: map[tailcfg.StableNodeID]ipn.PeerWireGuardState{
			"stable1": ipn.PeerWireGuardStateEstablished,
		},
	}
	...
	if n := deliver(ipn.NotifyPeerWireGuardState); n.PeerState["stable1"] != ipn.PeerWireGuardStateEstablished {
...
func TestPeerWireGuardStateWatchInitialThenDeltas(t *testing.T) {
	...
	b.onPeerWireGuardState(peer1.Key, wgengine.PeerWireGuardStateEstablished)
	b.onPeerWireGuardState(peer1.Key, wgengine.PeerWireGuardStateEstablished)
	defer b.onPeerWireGuardState(peer1.Key, wgengine.PeerWireGuardStateNone)
	defer b.onPeerWireGuardState(peer2.Key, wgengine.PeerWireGuardStateNone)
	...
	if got[0].PeerState["stable1"] != ipn.PeerWireGuardStateEstablished {
	...
	if len(got[1].PeerState) != 1 || got[1].PeerState["stable2"] != ipn.PeerWireGuardStateHandshake {
...
func TestPeerWireGuardStateMetrics(t *testing.T) {
	...
	defer b.onPeerWireGuardState(peer.Key, wgengine.PeerWireGuardStateNone)
	...
	b.onPeerWireGuardState(peer.Key, wgengine.PeerWireGuardStateHandshake)
	...
	b.onPeerWireGuardState(peer.Key, wgengine.PeerWireGuardStateExpired)
	...
	b.onPeerWireGuardState(peer.Key, wgengine.PeerWireGuardStateNone)
